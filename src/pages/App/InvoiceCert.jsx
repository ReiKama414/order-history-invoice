		...
		<div className="pt-9">
			<div>
				...
				<div className="text-sm leading-6 pb-[15px]">
					Only product sales records since January 1, 2021 are shown. All receipts must be based on physical receipts
					for our maintenance and buyback services in all Chow Tai Fook branches in Hong Kong and Macau.
				</div>
				{/* Divide - start */}
				...
				{data.map((item, index) => (
					<div key={index}>
						{/* Date of Purchase */}
						<h5 className="text-base font-bold pb-6 pt-4">{`Date of Purchase: ${item.date}`}</h5>
							...
							{item.invoices.slice(0, 3).map((invoice, invoiceIndex) => (
								<div key={invoiceIndex} className="border border-border rounded-[10px] text-base overflow-hidden">
									<div className="flex flex-col items-center p-4 pb-1.5">
										...
										<div className="font-bold text-center break-words whitespace-pre-line h-12">
											{invoice.storeInfo}
										</div>
									</div>
									{/* Invoice Button */}
									<div className="bg-customColor2 flex items-center">
										{invoice.type === "diamond" && (
											<>
												<Button className="underline underline-offset-2 text-sm leading-6 px-0" fullWidth>
													DIAMOND CERT
												</Button>
				...
				<div className="w-full xl:pt-8 pt-5 pb-4">
					<div className="relative w-fit mx-auto">
						<Button className="rounded-none pt-3 pb-3 border-2 border-white bg-customColor2 text-customColor2 font-normal text-sm leading-[18px] w-[182px]">
							MORE INVOICES
						</Button>
